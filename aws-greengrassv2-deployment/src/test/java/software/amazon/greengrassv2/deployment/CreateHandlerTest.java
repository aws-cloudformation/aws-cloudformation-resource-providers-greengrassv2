...
public class CreateHandlerTest extends AbstractTestBase {
    ...
    public void tear_down() {
        verify(iotClient, times(2)).serviceName();
    ...
    public void handleRequest_SimpleSuccess() {
        ...
        CreateThingResponse createThingResponse = CreateThingResponse.builder()
        ...
        doReturn(createThingResponse, describeThingResponse)
                .when(proxy)
                .injectCredentialsAndInvokeV2(ArgumentMatchers.any(), ArgumentMatchers.any());

        final ProgressEvent<ResourceModel, CallbackContext> response = handler.handleRequest(proxy, request, new CallbackContext(), proxyClient, logger);

        assertThat(response.getResourceModel().getTargetName()).isEqualTo(model.getTargetName());
